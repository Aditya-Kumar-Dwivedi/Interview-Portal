...
import com.example.InterViewPortal.Model.Data;
    ...
    private void InsertJob() {
        ...
        btn_post_job.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                String number = job_salary.getText().toString().trim();
                ...
                String test="2",test1="0";
                String[] digitsArray = number.split("");
                int c=0,f=0;
                for(int i=0;i<number.length();i++)
                {
                    if(digitsArray[i].compareTo(test)>0||c>1)
                    {
                        f=1;
                        break;
                    }
                    else if(digitsArray[i].compareTo(test1)!=0)
                        c++;
                }
                if(f==0) {
                    job_salary.setError("Number must be greater than 2");
                    return;
                }
                ...
                Data data = new Data(title, description, skills, number, id, date);
